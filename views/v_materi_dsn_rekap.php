 ...
 <script type="text/javascript">
   $.fn.dataTable.ext.search.push(
    function( settings, data, dataIndex ) {
        var min = $('#min').val();
         var max = $('#max').val();
        var age =  data[16]; // use data for the age column
        var age2 =  data[15];
        var age3 =  data[17];
        var kosong = '';
           if (max==kosong && min==kosong  )  {
       
            if (kosong==age3) {
              return true;
            }
        } else if (max!=kosong && min!=kosong) {
           if (max == age2 && min ==age) {
   ...
   $(document).ready(function () {
    ...
    // Event listener to the two range filtering inputs to redraw on input
           ...
           $('#data-table').DataTable( {
                "ajax": {
                    "url": "<?php echo base_url('C_bimbingan/getdata_materi_rekap') ?>",
                    "dataSrc": ""
                },
                "columns": [         
                                           {"data": "number"}, 
